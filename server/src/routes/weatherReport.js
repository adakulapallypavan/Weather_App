...
router.post("/", async (req, res) => {
  try {
    const weatherReport = new WeatherReport({ ...req.body, lastUpdated: Date.now() });
...
router.get("/", async (req, res) => {
  try {
    ...
    res.status(200).json(weatherReports); // lastUpdated will be included automatically
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.status(200).send(weatherReport); // lastUpdated will be included automatically
  } catch (error) {
    res.status(500).send(error);
  }
});
...
router.patch("/:id", async (req, res) => {
  ...
  const allowedUpdates = [
    ...
    "windSpeed",
    "dateTime",
    "lastUpdated" // Allow updates to lastUpdated if needed
  ];
  ...
  try {
    ...
    weatherReport.lastUpdated = Date.now(); // Update lastUpdated timestamp
    await weatherReport.save();
